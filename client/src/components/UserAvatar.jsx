...
import { logout } from "./../redux/slices/authSlice"; // Assuming authSlice.js is in the same directory
const UserAvatar = () => {
  ...
  const logoutHandler = async () => {
    try {
      const response = await fetch("http://localhost:8800/api/user/logout", {
        // Assuming your backend logout endpoint
        method: "POST", // Adjust to your backend requirements (POST/GET)
        credentials: "include", // Send cookies for backend validation
      });
      ...
      window.location.href = "/log-in"; // Redirect to login page (replace with appropriate path)
    } catch (error) {
